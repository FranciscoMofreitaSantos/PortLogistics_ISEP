...
export async function apiPatchVesselByIMO(
    ...
    const res = await webApi.patch(`/api/Vessel/imo/${imo}`, data);
    return res.data;
}

export async function apiDeleteVessel(id: string): Promise<void> {
    await webApi.delete(`/api/Vessel/${id}`);
}
