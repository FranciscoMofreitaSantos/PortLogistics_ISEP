...
interface Props {
    ...
    onDelete: (it: IncidentType) => void;
    onSelect: (it: IncidentType) => void;
    selectedCode: string | null;
}

function IncidentTypeTable({ items, onEdit, onDelete, onSelect, selectedCode }: Props) {
                  ...
                  {t(`incidentType.severity.${it.severity}`)}
                        ...
                        <td onClick={(e) => e.stopPropagation()}>
                            <div style={{ display: "flex", gap: "0.5rem" }}>
                                <button onClick={() => onEdit(it)} className="pr-edit-button" type="button">
