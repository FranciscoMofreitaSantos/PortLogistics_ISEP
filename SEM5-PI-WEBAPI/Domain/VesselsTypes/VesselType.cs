...
public class VesselType : Entity<VesselTypeId>, IAggregateRoot
{
    ...
    public void ChangeDescription(string? updatedDescription) =>
        SetDescription(updatedDescription ?? DefaultDescription);

    public void ChangeDimensions(int updatedMaxBays, int updatedMaxRows, int updatedMaxTiers)
    {
        SetDimensions(updatedMaxBays, updatedMaxRows, updatedMaxTiers);
    }

    public void ChangeMaxBays(int updatedMaxBays)
    {
        SetMaxBays(updatedMaxBays);
    ...
    private void SetName(string name)
    {
        if (string.IsNullOrWhiteSpace(name))
            throw new BusinessRuleValidationException("Name can't be empty.");

        if (name.Length > MaxNameLength)
            throw new BusinessRuleValidationException($"Name exceeds maximum length for a Vessel Type Name [MAX {MaxNameLength} characters].");
    ...
    private void SetDescription(string description)
    {
        if (description.Length > MaxDescriptionLength)
            throw new BusinessRuleValidationException($"Description exceeds maximum length for a Vessel Type Description [MAX {MaxDescriptionLength} characters].");

        if (description.Length < MinDescriptionLength)
            throw new BusinessRuleValidationException($"Description requires a minimum length for a Vessel Type Description of [MIN {MinDescriptionLength} characters].");
    ...
    {
        if (maxBays < MinBays)
            throw new BusinessRuleValidationException($"Max 'Bays' must be greater than {MinBays}.");
    ...
    {
        if (maxRows < MinRows)
            throw new BusinessRuleValidationException($"Max 'Rows' must be greater than {MinRows}.");
    ...
    {
        if (maxTiers < MinTiers)
            throw new BusinessRuleValidationException($"Max 'Tiers' must be greater than {MinTiers}.");
