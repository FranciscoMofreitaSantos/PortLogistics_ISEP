Write-Output "🧨 Resetting PostgreSQL database (safe mode, no superuser needed)..."

# Connection details
$DB_HOST = "10.9.21.87"
$DB_PORT = "5432"
$DB_NAME = "ThPA"
$DB_USER = "makeitsimple_user"
$DB_PASS = "3dj03"

# Definindo variável de ambiente PGPASSWORD para autenticação automática
$env:PGPASSWORD = $DB_PASS

Write-Output "🗑️ Cleaning schema 'public' (removing all tables, views, sequences)..."

# IMPORTANTE: usar here-string literal (@' ... '@) para não interpolar $$ nem \r\n
$SQL = @'
DO $$
DECLARE
    r RECORD;
BEGIN
    -- Drop all tables
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') LOOP
        EXECUTE 'DROP TABLE IF EXISTS ' || quote_ident(r.tablename) || ' CASCADE';
    END LOOP;

    -- Drop all views
    FOR r IN (SELECT viewname FROM pg_views WHERE schemaname = 'public') LOOP
        EXECUTE 'DROP VIEW IF EXISTS ' || quote_ident(r.viewname) || ' CASCADE';
    END LOOP;

    -- Drop all sequences
    FOR r IN (SELECT sequencename FROM pg_sequences WHERE schemaname = 'public') LOOP
        EXECUTE 'DROP SEQUENCE IF EXISTS ' || quote_ident(r.sequencename) || ' CASCADE';
    END LOOP;
END
$$ LANGUAGE plpgsql;
'@

psql -v ON_ERROR_STOP=1 -h $DB_HOST -U $DB_USER -d $DB_NAME -c $SQL

Write-Output "🧹 Removing old EF Core migrations..."
Remove-Item -Recurse -Force ./Migrations -ErrorAction SilentlyContinue

Write-Output "📦 Creating new EF Core migration..."
dotnet ef migrations add Initial

Write-Output "🛠️ Applying migration to clean database..."
dotnet ef database update --connection "Host=$DB_HOST;Port=$DB_PORT;Database=$DB_NAME;Username=$DB_USER;Password=$DB_PASS"

Write-Output "🔍 Verifying created tables..."
psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "\dt"

Write-Output "✅ Database fully cleaned and rQuilt!"